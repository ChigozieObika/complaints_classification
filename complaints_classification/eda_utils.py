...
class EdaDfPlotReady():
    ...
    def date_plot(self):
        self.group_by.rename(
            columns={
                'category': 'count_by_date'},
            inplace=True)
    ...
    def category_plot(self):
        self.group_by.rename(
            columns={
                'category': 'count_by_state'},
            inplace=True)
        self.group_by.sort_values(
            by='count_by_state',
            ascending=False,
            inplace=True)
        return self.group_by
    ...
    def top_states_count_df(self, category=None):
        if category is not None:
            ...
            top_states_df.sort_values(
                by='count_by_state', ascending=False, inplace=True)
...
class Plot():
    ...
    def plot_percent_count_top_states(self, category):
        ...
        sns.barplot(
            ...
            errwidth=0)
        plt.title(
            f'Percentage Count of {category} by States',
            fontsize=12,
            fontweight='bold')
        ...
        plt.savefig(
            f'image_files/Percentage Count of {category} by States.png')
        plt.show()

    def plot_category_trend(self):
        _, ax = plt.subplots(nrows=3, ncols=2, figsize=(20, 20))
        sns.lineplot(data=self.df.drop(['category'],
                                       axis=1),
                     x='date',
                     y='count_by_date',
                     color='blue',
        ...
        ax[0, 0].set_title(
            'Count of Complaints by Date - Five Categories', fontsize=14, fontweight='bold')
        sns.lineplot(data=self.df[self.df['category'] == 'billings'], x='date',
                     y='count_by_date', color='green', linewidth=2.5, ax=ax[0, 1])
        ax[0, 1].set_title(
            'Count of Complaints by Date - Billings', fontsize=14, fontweight='bold')
        ...
        ax[1,
           0].set_title('Count of Complaints by Date - Poor Customer Servcies',
                        fontsize=14,
                        fontweight='bold')
        ax[1, 0].set_ylim(0, 100)
        sns.lineplot(data=self.df[self.df['category'] == 'data_caps'], x='date',
                     y='count_by_date', color='purple', linewidth=2.5, ax=ax[1, 1])
        ax[1, 1].set_title(
            'Count of Complaints by Date - Data Caps', fontsize=14, fontweight='bold')
        ...
        ax[2, 0].set_title(
            'Count of Complaints by Date - Internet Probelms', fontsize=14, fontweight='bold')
        ax[2, 0].set_ylim(0, 100)

        sns.lineplot(data=self.df[self.df['category'] == 'other'],
                     x='date', y='count_by_date', linewidth=2.5, ax=ax[2, 1])
        ax[2, 1].set_title('Count of Complaints by Date - Other',
