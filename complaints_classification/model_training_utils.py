...
class PrepareData():
    ...
    def modify(self):
        ...
        self.df['category'] = self.df['category'].replace(
            ['Billings', 'Internet Problems', 'Poor Customer Service', 'Data Caps', 'Other', 'other'], [0, 1, 2, 3, 4, 4])
        self.df['state'] = self.df['state'].replace(
...
def process_text(text_series):
    ...
    df["lemmatized"] = df["no_stops"].apply(
    ...
    # delete rare words
    df["processed"] = df["lemmatized"].apply(
        lambda x: [word for word in x if word not in rare_words.keys()])
...
def deploy_plot(filepath, modelfile='lgr_model.pkl'):
    ...
    grouped_by_data = test_input.groupby(['date', 'predictions'])[
        'predictions'].count()
    grouped_by_data = pd.DataFrame(grouped_by_data)
    grouped_by_data.rename(
        columns={
            'predictions': 'number of complaints'},
        inplace=True)
    grouped_by_data.reset_index(inplace=True)
    sns.lineplot(
        data=grouped_by_data.drop(
            ['predictions'],
            axis=1),
        x='date',
